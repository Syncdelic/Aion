...
def parse_dates(fechas):
    # Regular expressions for date formats
    regex1 = re.compile(r"(\d{1,2}) de (\w+) de (\d{4})")
    regex2 = re.compile(r"(\d{4})-(\d{2})-(\d{2})")
    
    # Try to match regex1 (dd de MMMM de yyyy)
    match = regex1.findall(fechas)
    if match:
        try:
            start_date = datetime.strptime(f"{match[0][0]} de {match[0][1]} de {match[0][2]}", "%d de %B de %Y").strftime("%Y-%m-%d")
            end_date = datetime.strptime(f"{match[1][0]} de {match[1][1]} de {match[1][2]}", "%d de %B de %Y").strftime("%Y-%m-%d")
            return start_date, end_date
        except ValueError:
            pass

    # Try to match regex2 (yyyy-MM-dd)
    match = regex2.findall(fechas)
    if match:
        try:
            start_date = datetime.strptime(f"{match[0][0]}-{match[0][1]}-{match[0][2]}", "%Y-%m-%d").strftime("%Y-%m-%d")
            end_date = datetime.strptime(f"{match[1][0]}-{match[1][1]}-{match[1][2]}", "%Y-%m-%d").strftime("%Y-%m-%d")
            return start_date, end_date
        except ValueError:
            pass

    raise ValueError(f"Error parsing dates: {fechas}")

def extract_details(response_content):
    details = {}
    # Regular expressions to extract details
    name_regex = re.compile(r"Nombre: ([\w\s]+)")
    num_people_regex = re.compile(r"Número de Personas: (\d+)")
    fechas_regex = re.compile(r"Fechas: ([\d\s\w\-]+al[\d\s\w\-]+)")
    room_type_regex = re.compile(r"Tipo de Habitación: ([\w\s\d]+)")
    price_per_night_regex = re.compile(r"Precio por Noche: ([\w\s\d]+)")
    total_cost_regex = re.compile(r"Costo Total: ([\w\s\d\(\)]+)")

    # Extracting details using regex
    details["Nombre"] = name_regex.search(response_content).group(1).strip() if name_regex.search(response_content) else None
    details["Número de Personas"] = num_people_regex.search(response_content).group(1).strip() if num_people_regex.search(response_content) else None
    details["Fechas"] = fechas_regex.search(response_content).group(1).strip() if fechas_regex.search(response_content) else None
    details["Tipo de Habitación"] = room_type_regex.search(response_content).group(1).strip() if room_type_regex.search(response_content) else None
    details["Precio por Noche"] = price_per_night_regex.search(response_content).group(1).strip() if price_per_night_regex.search(response_content) else None
    details["Costo Total"] = total_cost_regex.search(response_content).group(1).strip() if total_cost_regex.search(response_content) else None
...
def whatsapp_reply():
    ...
    try:
        ...
        details = extract_details(response_content)
        ...
        if fechas:
            try:
                start_date, end_date = parse_dates(fechas)
