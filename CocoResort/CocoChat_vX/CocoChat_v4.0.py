...
import datetime as dt  # Alias to avoid naming conflict
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                Eres un asistente virtual para Coco Resort. Tu trabajo es proporcionar información sobre las opciones de alojamiento, los servicios del resort, la ubicación, el menú del restaurante y hacer reservas para los huéspedes. Sé amable y servicial. La fecha y hora actuales son {dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}.
        ...
        if "reserva" in incoming_msg.lower() or "reservar" in incoming_msg.lower():
            ...
            if details:
                ...
                # Create reservation
                customer = Customer(1, details.get("Nombre"), user_number)
                room_type = details.get("Tipo de Alojamiento")
                rooms_available = [room for room in hotel_system.rooms if room.type == room_type and room.is_available]

                if rooms_available and start_date and end_date:
                    room = rooms_available[0]
                    reservation = Reservation(1, room, customer, start_date, end_date, details.get("Número de Personas"))
                    ...
                    add_reservation(
                        csv_file_path,
                        convert_reservation_to_dict(reservation)
                    )
                    response_msg += "\n\nSu reserva ha sido añadida."
                else:
                    print("Incomplete reservation details:")
                    print(f"Customer: {customer.name}, Room: {room_type}, Start Date: {start_date}, End Date: {end_date}")
